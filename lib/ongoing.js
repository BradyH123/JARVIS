 ...
 */
// Never let ongoing tasks pile up: at most this many run concurrently, and the
// same goal never spawns a duplicate. Without this, a schedule (or repeated
// routing) that keeps firing an open-ended "monitor X" request would create a
// new never-ending task every time and flood the machine.
const MAX_CONCURRENT = 3;

function runningTasks() {
  return [...tasks.values()].filter((t) => t.status === 'ongoing');
}
...
function start(goal, opts, deps) {
  ...
  // Dedupe: if a task with the same goal is already running, return it instead
  // of starting another copy.
  const running = runningTasks();
  const dup = running.find((t) => slugify(t.goal) === slugify(g));
  if (dup) return { ...snapshot(dup), alreadyRunning: true };
  // Cap: refuse to exceed the concurrency limit (the user must stop some first).
  if (running.length >= MAX_CONCURRENT) {
    return { error: `Already running ${running.length} ongoing tasks (the max). Say "stop" to clear them before starting another.`, atCapacity: true };
  }
  ...
  const task = {
    ...
    // Include the unique id so two same-goal tasks on the same day never share
    // a note/report file (which would interleave findings and clobber reports).
    notePath: path.join(opts.notesDir, `${slugify(g)}-${new Date().toISOString().slice(0, 10)}-${id}.md`),
    reportPath: null,
    lastFinding: '',
  };
  tasks.set(id, task);
