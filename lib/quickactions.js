...
/**
 * Find a RUNNING app process matching the query fuzzily — so "Ableton Live"
 * resolves to the process actually named "Live" or "Ableton Live 12 Suite".
 */
async function findRunningProcess(query) {
  const r = await runOut('osascript', ['-e', 'tell application "System Events" to get name of every application process']);
  const procs = (r.output || '').split(',').map((s) => s.trim()).filter(Boolean);
  const words = String(query).toLowerCase().split(/\s+/).filter((w) => w.length > 2);
  let best = null;
  let bestScore = 0;
  for (const p of procs) {
    if (SELF_NAMES.test(p)) continue;
    const pl = p.toLowerCase();
    let score = words.filter((w) => pl.includes(w)).length;
    if (pl === String(query).toLowerCase()) score += 5;
    if (score > bestScore) {
      bestScore = score;
      best = p;
    }
...
/** Quit an application by name. Fuzzy-resolves the real running app. Never quits JARVIS. */
async function quitApp(name) {
  ...
  // Resolve the actual running app name: exact → installed bundle → running
  // process (handles "Ableton Live" → "Ableton Live 12 Suite" / "Live").
  let real = app;
  if (!(await isRunning(real))) {
    const bundle = await findAppBundle(app);
    if (bundle) real = path.basename(bundle, '.app');
  }
  if (!(await isRunning(real))) {
    const proc = await findRunningProcess(app);
    if (proc) real = proc;
    else return { ok: true, text: `${app} isn't running.` };
  }
  if (SELF_NAMES.test(real)) return { ok: false, error: "I won't quit myself." };
  const r = await runOut('osascript', ['-e', `tell application "${real.replace(/"/g, '')}" to quit`]);
  return r.ok ? { ok: true, text: `Quit ${real}.` } : { ok: false, error: r.error || 'quit failed' };
}
