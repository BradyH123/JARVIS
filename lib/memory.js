...
/** The last N conversation lines from today's note (for short-term recall). */
function recentConversation(maxLines = 16) {
  const raw = readSafe(path.join('Conversations', today() + '.md'));
...
function contextForPrompt() {
  ...
  const recent = recentConversation(16);
  if (recent.length) parts.push("Today's conversation so far:\n" + recent.join('\n'));
