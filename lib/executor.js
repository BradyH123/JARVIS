...
function loadNut() {
  ...
  nut.mouse.config.autoDelayMs = 6; // a touch slower = far more reliable clicks
  nut.keyboard.config.autoDelayMs = 2;
  return nut;
}

const config = require('./config');
const frontmost = require('./frontmost');
...
// Let the cursor settle at its destination before a click fires — clicking the
// same instant the pointer is warped often lands on the OLD position or misses.
const MOVE_SETTLE_MS = 45;
...
async function perform(action) {
  ...
  try {
    switch (a) {
      ...
      case 'key': {
        // Mechanical self-preservation: never let a quit / close-window shortcut
        // fire while JARVIS's own window is frontmost.
        const combo = String(action.text || '').toLowerCase().replace(/\s/g, '');
        if (/(cmd|command|super|meta)\+(shift\+)?q$|(cmd|command|super|meta)\+w$/.test(combo)) {
          if (await frontmost.selfIsFrontmost()) {
            return { ok: false, error: 'Refused: that shortcut would quit/close JARVIS itself.' };
          }
        }
