...
function tick(nowMs, execute) {
  ...
  for (const j of [...jobs]) {
    ...
    if (j.spec.kind === 'once') {
      ...
      j.nextAt = computeNext(j.spec, now);
    }
    try {
...
module.exports = { init, add, list, remove, clear, tick, startTicking, stopTicking, computeNext, normalizeWhen, describe };
