...
const READ_JS = `(function(){
  ...
  // Clear refs from a previous read so stale [data-jarvis-ref] can't make a
  // click by ref land on the wrong (old) element after a same-page re-read.
  var old=document.querySelectorAll('[data-jarvis-ref]');
  for(var o=0;o<old.length;o++){old[o].removeAttribute('data-jarvis-ref');}
...
async function run(goal, opts = {}) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    // Act on the first tool call; every OTHER tool_use in the same turn still
    // needs a tool_result or the next request is rejected ("tool_use ids were
    // found without tool_result blocks"). Extras get a benign one-at-a-time note.
    const call = toolUses[0];
    const extraResults = toolUses.slice(1).map((tu) => ({
      type: 'tool_result',
      tool_use_id: tu.id,
      content: [{ type: 'text', text: 'Skipped — I do one browser action at a time; reissue this if still needed.' }],
    }));
