...
async function routeCommand(transcript, skillsContext, workflowsContext) {
  ...
  const tools = [
    ...
    },
    {
      ...
      description:
        'Set up a CONTENT-REACTIVE STRATEGY: a recurring task that reads an external source and ' +
        'then schedules NEW tasks based on the specific content it finds. Use for requests like ' +
        '"for the next 2 months every day look at the news and schedule tasks to look into the ' +
        'stories deeper and build reports", "each morning read TechCrunch and queue write-ups on ' +
        'the biggest launches". Extract: the SOURCE to read, the standing INSTRUCTION (what to do ' +
        'with what it finds), the cadence, and the total duration.',
      input_schema: {
        type: 'object',
        properties: {
          source: { type: 'string', description: 'What to read each time, e.g. "the news", "techcrunch.com".' },
          instruction: { type: 'string', description: 'What to do with what it finds, e.g. "look into the top stories deeper and build reports".' },
          when_kind: { type: 'string', enum: ['daily', 'weekly', 'every'], description: 'Cadence.' },
          time: { type: 'string', description: 'For daily/weekly: local HH:MM (24h). Default 08:00.' },
          weekday: { type: 'number', description: 'For weekly: 0=Sunday…6=Saturday.' },
          minutes: { type: 'number', description: 'For every: minutes between runs.' },
          duration_days: { type: 'number', description: 'How many DAYS the strategy runs (e.g. 60 for two months).' },
        },
        required: ['source', 'instruction', 'when_kind', 'duration_days'],
      },
    },
    {
  ...
  if (call.name === 'create_strategy')
    return {
      action: 'create_strategy',
      source: call.input.source,
      instruction: call.input.instruction,
      when: { kind: call.input.when_kind, time: call.input.time, weekday: call.input.weekday, minutes: call.input.minutes },
      durationMinutes: Math.max(0, Math.round((Number(call.input.duration_days) || 0) * 24 * 60)),
    };
...
/**
 * Distill a LESSON — the user just demonstrated something on screen while
 * JARVIS watched. Extract exactly what was shown: the ordered workflow, the
 * durable interface patterns, and (when it's a repeatable technique) a skill
 * definition JARVIS can replay later. This is the heart of "teach JARVIS an
 * app by showing him".
 *
 * @param {string[]} frames   sampled screen frames of the demonstration, in order
 * @param {string} question   what the lesson was about ("how to sidechain in Ableton")
 * @param {string} [app]      the app being taught, if known
 */
async function distillLesson(frames, question, app) {
  const client = getClient();
  const images = (frames || []).map(frameToImageBlock).filter(Boolean).slice(0, 20);
  if (!images.length) return { ok: false, error: 'No frames captured during the lesson.' };
  const tool = {
    name: 'report_lesson',
    description: 'Report what the demonstration taught.',
    input_schema: {
      type: 'object',
      properties: {
        app: { type: 'string', description: 'The application that was demonstrated (e.g. "Ableton Live").' },
        summary: { type: 'string', description: 'One sentence: what the user showed.' },
        workflow: { type: 'array', items: { type: 'string' }, description: 'The ordered steps the user performed, each a concrete action phrase ("click the Sends knob on track 2").' },
        patterns: { type: 'array', items: { type: 'string' }, description: 'Durable interface knowledge visible in the demo: where controls live, icons, navigation idioms. One sentence each.' },
        skill_worthy: { type: 'boolean', description: 'true if this is a repeatable technique worth saving as a named skill.' },
        skill_name: { type: 'string', description: 'Short name for the skill, if skill_worthy.' },
        unclear: { type: 'string', description: 'What (if anything) was unclear from the demo — a follow-up question to ask the user. Empty if all clear.' },
      },
      required: ['summary', 'workflow', 'patterns', 'skill_worthy'],
    },
  };
  const message = await client.messages.create({
    model: config.getModel(), // vision quality matters for a one-shot lesson
    max_tokens: 1200,
    tools: [tool],
    tool_choice: { type: 'tool', name: 'report_lesson' },
    messages: [
      {
        role: 'user',
        content: [
          {
            type: 'text',
            text:
              `The user just TAUGHT you: "${String(question || 'how this app is used')}"` +
              (app ? ` in ${app}` : '') +
              '. These frames show their demonstration, in order. Extract exactly what they ' +
              'showed — the ordered steps and the durable interface knowledge — precisely ' +
              'enough that you could repeat it yourself. Only report what you can actually see.',
          },
          ...images,
        ],
      },
  ...
  const call = (message.content || []).find((b) => b.type === 'tool_use');
  if (!call || !call.input) return { ok: false, error: "I couldn't make sense of the demonstration." };
  return { ok: true, ...call.input };
}

/**
 * Content-reactive strategy: given a block of freshly-read external content
 * (e.g. this morning's news) and the user's standing instruction ("look into
 * the stories deeper and build reports"), derive SPECIFIC follow-up tasks tied
 * to the ACTUAL content — one per concrete item, phrased so JARVIS can carry it
 * out. This is what lets a daily strategy task spawn different work each day
 * based on what it actually finds.
 *
 * @param {string} content     the freshly-read source text
 * @param {string} instruction the user's standing strategy ("research each top story and write a report")
 * @param {object} [ctx]        { max?, alreadyDone?: string[] }
 * @returns {Promise<{summary:string, tasks:Array<{command:string, why:string}>}>}
 */
async function deriveTasksFromContent(content, instruction, ctx = {}) {
  const client = getClient();
  const max = Math.max(1, Math.min(Number(ctx.max) || 3, 6));
  const tool = {
    name: 'report_derived',
    description: 'Report specific follow-up tasks derived from the content.',
    input_schema: {
      type: 'object',
      properties: {
        summary: { type: 'string', description: 'One sentence: what stood out in the content today.' },
        tasks: {
          type: 'array',
          description:
            `Up to ${max} SPECIFIC tasks following the user's instruction, each tied to a concrete ` +
            'item in the content (name the actual story/topic). Each is a command JARVIS can run ' +
            '(research X deeply and write a report, look into Y, summarize Z). Skip vague items.',
          items: {
            type: 'object',
            properties: {
              command: { type: 'string', description: 'e.g. "Research the [specific headline] story in depth and write a report on it".' },
              why: { type: 'string' },
            },
            required: ['command'],
          },
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 1200,
    tools: [tool],
    tool_choice: { type: 'tool', name: 'report_derived' },
    system:
      IDENTITY +
      '\n\nYou are executing a standing STRATEGY: read fresh external content and turn what you ' +
      'find into specific, concrete follow-up tasks tied to the actual items in it. Be selective ' +
      "and specific — name the real stories/topics. If nothing warrants a task, return empty.",
    messages: [
      {
        role: 'user',
        content:
          `Standing instruction: ${String(instruction || 'derive useful follow-up tasks').slice(0, 400)}\n\n` +
          (ctx.alreadyDone && ctx.alreadyDone.length
            ? `Already covered (do NOT repeat):\n- ${ctx.alreadyDone.slice(-40).join('\n- ')}\n\n`
            : '') +
  ...
  const tasks = (Array.isArray(call.input.tasks) ? call.input.tasks : [])
    .filter((t) => t && t.command && String(t.command).trim())
    .slice(0, max)
    .map((t) => ({ command: String(t.command).trim(), why: String(t.why || '').trim() }));
  return { summary: String(call.input.summary || '').trim(), tasks };
}
